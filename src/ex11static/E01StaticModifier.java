...
/*
 ...
 - 정적변수는 JVM(자바가상머신)에 의해 프로그램이 종료되는 시점에 초기화된다.
 
 -main() 메소드가 실행되기 전에 이미 초기화되므로 main메소드 실행 시 별도의 선언없이 변수를 사용할 수 있다.
...
class MyStatic{
	...
	/*
	 ...
	 프로그램 시작 시 이미 메소드영역에 초기화된다.
	 */
	static int staticVar;
	
	
	/*
	 인스턴스형 멤버메소드] 인스턴스형 메소드에서는 모든 멤버를 사용할 수 있다. 인스턴스형, 정적변수 둘다 접근가능함. 
	 */
	void instanceMethod() {
	...
	/*
	 정적 멤버메소드] 정적메소드 안에서는 인스턴스 멤버를 사용할 수 없다. 정적멤버만 접근 가능하다.
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		/*
		 일반적인 멤버변수에 접근하는 방법 ->
		 MyStatic 믈래스의 객체를 생성한 후 참조변수를 통해 접근한다.
		 */
		...
		/*
		 정적 멤버변수는 각각의 객체마다 멤버변수가 존재하지 않고, 메소드영역에 하나만 생성되어 공유되므로, 
		 아래 대입문장은 하나의 변수에 값이 할당되게 된다. 따라서 결과는 둘다 900이 출력된다. 
		 */
		...
		myStatic.staticMethod(); // 권장사항이 아니므로 클래스명을 통해 접근하는 것이 좋음.
