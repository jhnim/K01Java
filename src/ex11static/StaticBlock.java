...
/*
 static블럭
 	: 동일 클래스의 main메소드보다 먼저 실행되는 블럭으로 main메소드의 실행코드가 없어도 먼저 실행된다.
 	단, 다른 클래스에 main이 있는 경우에는 main이 순차적으로 실행되다가 static블럭이 있는 클래스가 객체화될 때 실행된다.
...
public class StaticBlock {
	...
	static {
		...
		/*
		 블럭 내에서만 사용할 수 있는 변수로써 이때는 일반적인 변수를 생성할 수 있다.
		 */
		int localVar;
		localVar = 1000;
	...
	//생성자메소드
	/*
	 외부파일의 main메소드에서 실행되는 경우
	  해당 클래스의 갹채가 생성자를 통해 생성되기 전에 static블럭이 먼저 실행된다.
	 */
	public StaticBlock() {
		staticVar = -1;
	...
	public static void name(String[] args) {
		...
		//static블럭 내에서 선언된 지역변수이므로 main에서는 사용불가
