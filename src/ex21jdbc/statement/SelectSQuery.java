...
public class SelectSQuery extends ConnectDB {
	...
	/*
	 ResultSet 클래스
	 	: select문 실행시 뭐리의 실행결과가 resultSet객체에 저장된다. 결과로 반환된 레코드의 처음부터 마지막까지 next()메소드를 통해
	 	확인 후 반복하면서 추출하게 된다.
	 	- getXXX()계열의 메소드
	 	오라클의 자료형이
	 		number타입 : getInt()
	 	...
	 	- 인자는 select절의 컬럼순서대로 인덱스(1부터 시작)를 사용하거나 컬럼명을 사용할 수 있다.
	 */
	@Override
	public void execute() {
	...
	public static void main(String[] args) {
		SelectSQuery selectSQL1 = new SelectSQuery();
		selectSQL1.execute();
	}
