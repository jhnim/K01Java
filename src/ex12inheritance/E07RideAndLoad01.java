...
/*
 ...
 오버로딩: 함수명은 같으나 매개변수의 갯수, 타입, 순서가 다른 경우를 말한다.
 	즉, 서로 다른 함수이므로 상속받은 하위클래스에서는 오버로딩된 모든 메소드가 포함된다.
 	(상속의 된다)
 
 오버라이딩 : 함수명, 매개변수, 반환형까지 모두 동일한 함수를 상속관계에 있는 하위클래스에서 정의한 메소드가 상위클래스의 메소드를 가리게 되므로
 	항상 최하위 클래스의 메소드가 호출되게 된다. (결과적으론 상속이 되지 않음/ 덮어쓰고 존재자체가 사라짐)
...
public class E07RideAndLoad01 {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 즉, 아래 부분은 모두 C클래스에 생성된 메소드가 호출되게 된다.
		 */
