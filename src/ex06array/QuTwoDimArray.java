...
//이 메소드를 기반으로 int형 2차원 배열에 저장된 값을 증가시키는 프로그램을 작성하시오. 단 2차원 배열의 가로, 세로길이에 상관없이 동작해야 하고,
...
class QuTwoDimArray
{
	...
	{
		//세로에 대한 바녹을 통해 한행씩 addOneArr()로 전달한다. (이차원배열에서는 배열의 이름을 통해 세로길이를 알 수 있음)
		for(int i=0 ; i<arr.length ; i++) {
			addOneArr(arr[i], add);
		}
	}
