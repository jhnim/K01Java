...
public class PackageMain {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 방법2: 하나의 클래스는 import하고, 다른 하나는 풀경로를 사용한다.
		 */
		Date utilDate = new Date();//import 후 사용
		System.out.println("utilDate="+ utilDate);
		
		java.sql.Date sqlDate = new java.sql.Date(utilDate.getTime());//풀패키지의 풀경로
		System.out.println("sqlDate="+sqlDate);
		
		//////////////////////////////////////////////////////////
		/*
		 ex09package 하위에 ex09package.kosmo.util 패키지가 생성되지만 이 두개는 서로 다른 패키지로 인식한다.
		 따라서 isNumber() 메소드를 호출하기 위해서는 해당 패키지를 import해야한다.
		 */
		String strValue = "987654321";
		
		/*
		 static으로 선언되는 메소드는 new를 통한 객체생성없이 클래스명만으로 즉시 호출이 가능하다.
		 */
