	...
	public static void main(String[] args) {
		...
		/*
		 List<E> 인터페이스 계열의 컬렉션
		 	: ArrayList<T>와 같은 컬렉션은 제네릭을 기반으로 하지만 아래와 같이 타입매개변수를 생략하면 모든 객체를 저장할 수 있는 컬렉션이 된다.
		 	즉 ArratList<Object>로 선언한 것과 동일하다.
		 */
		...
		/*
		 1-4] 덮어쓰기
		 	set(인덱스, 객체) : 특정 인덱스에 저장된 데이터를 덮어쓰게되므로 기존자료는 삭제된다.
		 */
		...
		/*
		 1-5] 끼워넣기
		 	add(삽입할 인덱스, 객체) : 해당 인덱스에 삽입하고 기존데이터는 자동으로 뒤로 밀리게된다.
		 */
		...
		/*
		 1-6] 포함여부확인
		 	contains(객체) : 찾고자 하는 객체가 있는 경우 true를 반환한다. 위차한 인덱스까지는 확인할 수 없다.
		 */
		...
		/*
		 1-7] 삭제
		 방법1 : 인덱스로 삭제하기
		 	: remove(index) : 인덱스를 통해서 삭제를 진행한 후 삭제가 완료되면 해당 객체를 반환한다. 삭제후에는 인덱스가 자동으로 재부여된다.
		 	삭제 성공시 해당객체를 반환하므로 어떤 객체가 삭제되었는지 확인가능하다.
		 */
