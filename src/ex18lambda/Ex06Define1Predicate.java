...
import ex18lambda.Human;

/*
 JAVA에서 제공하는 기본 함수형 인터페이스는 다음과 같은 종류가 있다. 큰 차이라면 매개변수의 유무, 리턴값의 유무정도 이다.
...
public class Ex06Define1Predicate {
	...
	public static double avg(Predicate<Human> ph, List<Human> li) {
		...
		int pCount = 0; // 전달된 객체의 성별을 판단하여 인원 수를 카운트
		int tCount = 0; // 점수를 누적해서 더해 총점을 저장하기 위한 변수
		for(Human h : li) { // li컬렉션의 크기만큼 반복
			/*
			 첫번째 호출에서는 남자일때 true를 반환하므로 남자의 인원수와 점수가 카운트된다.
			 */
			if(ph.test(h)==true) {
				pCount++; // 인원수 증가
