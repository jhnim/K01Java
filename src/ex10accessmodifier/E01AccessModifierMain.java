...
public class E01AccessModifierMain {
	...
	public static void main(String[] args) {
		...
		/*
		 private으로 선언된 멤버는 동일한 패키지라도 접근이 불가능 하다.
		 해당 멤버는 동일 클래스 내에서만 접근을 허용한다.
		 
		 ※ private으로 선언된 멤버를 외부클래스에서 접근할 때는 주로 public으로 선언된 멤버메소드를 통해 간접적으로 접근한다.
		 */
		...
		//two.privateMethod(); //에러발생 //내꺼만
		//two.defaultMethod(); //에러발생 //같은 패키지 내에서만
		two.publicMethod();				// 개나소나
		
		/*
		 다른 패키지에 정의된 디폴트 클래스이므로 객체생성이 불가능하다.
		 import도 불가능하므로 이에 사용할 수 없는 클래스가 된다.
		 */
		System.out.println("DefaultClass 객체 생성 및 접근");
