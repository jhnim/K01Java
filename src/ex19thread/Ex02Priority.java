...
class MessageSendingThread extends Thread{
	...
	public void run() {
		for(int i=1 ; i<=1000 ; i++) {
			...
			/*
			 기본적으로 우선순위가 높은 쓰레드가 먼저 실행되지만 sleep()메소드를 사용하게 되면 
			 실행 중 잠깐씩 블럭상태로 전환되어 우선순위가 낮은 쓰레드도 가끔 실행될 수 있는 상황이 생기게 된다.
			 */
			try {
...
//		MessageSendingThread tr3 = new MessageSendingThread("세번째");
		...
		/*
		 2단계 실행
		 	: 우선순위 부여를 위해 정수를 사용한다. 우선순위는 1~10 사이에서 부여할 수 있고 높을수록 우선순위도 높아지게된다.
		 	※ 단 우선순위가 높다고 무조건 먼저 실행되지는 않는다. 전체적인 확률이 높아지게되어 먼저 실행된 가능성이 높아진다고 생각하면된다. 
		 */
