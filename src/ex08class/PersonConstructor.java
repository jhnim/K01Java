...
/*
생성자(Constructor)
-클래스를 객체화(인스턴스)할 깨 자동으로 호출되는 메소드
- 자동으로 호출되면, 개발자가 임의로 호출할 수 엇다.
- 반환값이 없다. 즉 특정 연산 후 값을 반활 할 수 없디. 따라서 void를 쓰지 않는다.
- 그외는 메소드가 가진 모든 특성을 가지고 있다.(매개변수 전달, 오버로딩 등)

 생성자 내에서 this사용법
 용도1: 동일 클래스 내에서 다른 생성자를 호출할 때 사용하나 단, 생성자 내에서만 호출 가능하다.
 	사용법:
 		this(인자1, 인자2,....);
 용도2: 생성자 내에서 멤버변수와 매개변수의 이름이 동일할 때 구분하기 위해 사용된다.
 	this.변수 => 멤버변수
 	변수=> 생성자의 매개변수
 	단, 이름이 서로 다르면 this는 생략해도 된다. 주로 명시적인 코드작성을 위해 사용된다.
 */
...
public class PersonConstructor {
	...
	public PersonConstructor(String name) {
		...
		this.name = name; // 좌측항은 멤버변수, 우측항은 매개변수 //this가 붙으면 멤버변수(클래스 입장에서 내꺼)
		age = 1;
		addr = "출처불명";
	...
	//멤버베소드: 멤버변수를 초기화 할 목적으로 정의한 메소드
