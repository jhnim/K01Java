...
public class E02StringMethod {
	...
	public static void main(String[] args) {
		...
		System.out.println("str1의 두번째 문자:"+ str1.charAt(1));// 첫번째 문자열의 2번째 문자
		...
		/*
		 10] indexOf()
		 	: 문자열에서 특정 문자열의 시작인덱스를 반환한다. 만약 찾는 문자열이 없다면 -1을 반환한다. 
		 */
		System.out.println(str1.indexOf("ava")); //12
		System.out.println(str1.indexOf("J")); //-1
		System.out.println(email.indexOf("@")!=-1 ? "이메일 형식 맞음" : "이메일 형식 아님");
		...
		/*
		 11] lastIndexOf()
		 	: 사용법은 indexOf()와 동일하나 문자열의 뒤에서 부터 문자열을 찾아서 인덱스를 반환한다.
		 */
		...
		/*
		 12] replace
		 	...
		 	만약 찾는 문자열이 없다면 변경만 되지 않을 뿐더러 에러가 발생하지는 않는다.
		 */
		...
		System.out.println(str1.replace("java", "KOSMO")); //KOSMO
		...
		/*
		 14] substring()
		 	...
		 	사용 1 : substring(index) -> 이 경우 index부터 문자열의 끝까지를 잘라서 반환함.
